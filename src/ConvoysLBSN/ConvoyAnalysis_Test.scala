...
class ConvoyAnalysis_Test {
  ...
  : Array[((Double, Double), scala.collection.mutable.HashMap[Long, Long], Long)] = {
    ...
    val ckLines = scala.io.Source.fromFile(checkinsFile).getLines()//.take(10000)
      ...
      //.filter(t=> t._1==12108|| t._1==4339 )//|| t._1==4339
    println("check-ins size::"+ckLines.size)
    //println("checkins are::")
    //ckLines.foreach(println)
  ...
  : Unit ={
    println("now function user Activities***********************************")
    ...
    groupActivities.groupBy(t=> t._1).foreach{gc=>
      ...
      for(i:Int<-1 until gc._2.size){
        ...
        if(newOne._2==previous._2 && newOne._1==previous._1 && newOne._3==(previous._3+1)){ //loc=loc, user=user,time=time+1
          ...
          /*if(previous._2==26795248748L || newOne._2==26795248748L){
            println(" Yes found it !!")
          }*/
          if(i== gc._2.size-1){ //last activity if not changed i.e.., t-1=t for t is last time stamp
            newGroups += ((newOne._1,newOne._2,timeList))
          ...
          if(i== gc._2.size-1){ //last activity  changed i.e.., t-1!=t for t is last time stamp
            newGroups += ((newOne._1,newOne._2,ListBuffer(newOne._3)))
            //println("inserted ::"+previous)
            //println("with time stamps::"+timeList)
