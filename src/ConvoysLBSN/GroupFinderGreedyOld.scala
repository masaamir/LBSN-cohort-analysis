...
class GroupFinderGreedyOld {
  ...
  */
  ...
  : ListBuffer[Long] ={
    ...
    val userActsMap=userActsTS.groupBy(t=> t._1) // group activities by user: List[(user, List[(user, location, List[cat], (startTime,endTime))])]
    println("user acts map size ::"+userActsMap.size)
    val userCatActMap=userActsMapToUserCatActsMap(userActsMap,inCat) //Map[(user,cat), ListBuffer[(user, loc, List[cat], (ts, te))]]
    ...
    val userScorePair=getUsersAffinity(users,userActsMap,userCatActMap,catsActsMap,inCat,lambda)
      .sortBy(t=> -t._2).toMap

    // pair activities
    val pairActs=findPairActs(groupActsCatsTSFile)
    val pairActsMap=pairActs.groupBy(t=> t._1)
    val pairCatsActs=findPairActsCats(pairActs)
    val pairCatsActsMap=pairCatsActs.groupBy(t=> t._1)
